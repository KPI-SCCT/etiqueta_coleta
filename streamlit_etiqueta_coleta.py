...
def main() -> None:
    ...
    with st.form("form_etiqueta", clear_on_submit=False):
        ...
        largura_mm = float(colc1.number_input("Largura (mm)", min_value=1.0, value=90))
        altura_mm = float(colc2.number_input("Altura (mm)", min_value=1.0, value=100))
        espacamento_linhas = float(
            colc3.number_input("Espaçamento linhas (pt)", min_value=0.0, value=5)
        )
